module.exports = (req, res, next) => {
    ...
    if (req.path === "/register") {
        if (![email, name, password, matricula].every(Boolean)) {
            ...
            return res.status(401).json({message: "El nombre solo debe contener letras y espacios."});
        } else if (!validMatricula(matricula)) {
            console.log("Invalid Matricula");
            return res.status(401).json({message: "La matrícula debe ser numérica y tener entre 4 y 9 caracteres."});
        } else if (!validPassword(password)) {
            console.log("Invalid Password");
            return res.status(401).json({message: "La contraseña debe tener entre 4 y 15 caracteres y solo contener letras y números."});
        }
